...
<<heatmap-{{method}}-{{filter}}-{{groups}}, opts.label = 'fig.large', eval = nrow(dat.filter)<=1500, fig.cap='Heatmap of Gene expression values for genes that survived a filter of {{filter}}\\%'>>=
